...
def create_box_plot(data_dict, y_col, title, y_title, layer):
    ...
    unique_conditions = list(set([wafer_conditions.get((wafer_id, layer), '') for wafer_id in sorted_wafer_ids]))
    ...
    if len(unique_conditions) > 1 and unique_conditions != ['']:
        ...
        for condition in unique_conditions:
            if condition:  # Only add non-empty conditions to legend
...
def generate_complete_summary_stats(selected_parameter=None):
    ...
    for (product, lot, layer), wafer_ids in wafer_groups.items():
        ...
        for wafer_id in sorted(wafer_ids):
            ...
            etest_data = df_etest_common[(df_etest_common['WAFER_ID'] == wafer_id) & 
                                 (df_etest_common[etest_column].notna())].copy()
...
def generate_plots(filter_option, selected_layer=None, selected_cd_site=None, selected_parameter=None, scale_option='auto'):
    ...
    # For ETest plots, we want to show ALL wafers regardless of selected layer
    # The selected layer is only used for determining Condition on X-axis
    # Group all wafers by Product/Lot for ETest display
    etest_wafer_groups = {}
    
    # Get all unique wafers with ETest data
    unique_etest_wafers = set(df_etest_common['WAFER_ID'].unique())
    
    # Group ETest wafers by Product/Lot from the existing wafer_groups
    for (product, lot, layer), wafer_ids in wafer_groups.items():
        key = (product, lot)  # Remove layer dependency for ETest grouping
        if key not in etest_wafer_groups:
            etest_wafer_groups[key] = set()
        
        # Add all wafers from this group that have ETest data
        for wafer_id in wafer_ids:
            if wafer_id in unique_etest_wafers:
                etest_wafer_groups[key].add(wafer_id)
    
    # Convert sets back to lists
    etest_wafer_groups = {k: list(v) for k, v in etest_wafer_groups.items()}

    for (product, lot), wafer_ids in etest_wafer_groups.items():
        # Use the selected parameter instead of layer-based selection
        etest_column = selected_parameter
        etest_title = f'{selected_parameter} (ETest)'
        ...
        for wafer_id in sorted(wafer_ids):
            ...
            if selected_layer:
                ...
                fccd_data = df_fccd_common[(df_fccd_common['WAFERID'] == wafer_id) & 
                                         (df_fccd_common['LAYER'] == selected_layer)]
                
                # Filter DCCD data for selected wafer and layer
                dccd_data = df_dccd_common[(df_dccd_common['WAFERID'] == wafer_id) & 
                                         (df_dccd_common['LAYER'] == selected_layer)]
        ...
        group_header = html.Div([
            html.H2(f"Product: {product} | Lot: {lot} ({len(valid_wafers)} wafers) - Conditions based on Layer: {selected_layer or 'All'}", 
        ...
        # ETest Box Plot
        if etest_data_dict:
            fig_etest = create_box_plot(etest_data_dict, etest_column, f'{etest_title} Distribution', etest_title, selected_layer)
            plot_divs.append(
                html.Div([
                    dcc.Graph(
                        figure=fig_etest, 
        ...
        # FCCD Box Plot
        if fccd_data_dict:
            fig_fccd = create_box_plot(fccd_data_dict, 'CD', 'FCCD Distribution', 'FCCD (nm)', selected_layer)
            plot_divs.append(
                html.Div([
                    dcc.Graph(
                        figure=fig_fccd, 
        ...
        # DCCD Box Plot
        if dccd_data_dict:
            fig_dccd = create_box_plot(dccd_data_dict, 'CD', 'DCCD Distribution', 'DCCD (nm)', selected_layer)
            plot_divs.append(
                html.Div([
                    dcc.Graph(
                        figure=fig_dccd, 
